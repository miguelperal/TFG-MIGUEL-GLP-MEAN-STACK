<template>
  <div class="entradas-container">
    ...
    <div class="search-add-container">
      ...
      <button @click="buscarEntradasPorUsuario">Buscar</button>
    </div>
    <div class="error-message" v-if="errorBusqueda">{{ mensajeError }}</div>

    <div v-for="publicacion in publicacionesUsuario" :key="publicacion.id">
      <div>
        ...
        <button @click="deleteEntrada(publicacion.titulo)">Eliminar Entrada</button>
      </div>
      <hr>
    </div>
  </div>
  ...
  <div class="form-container" v-if="mostrarFormularioNuevaEntrada">
    ...
    <form @submit.prevent="submitEntrada(entradaForm)">
      ...
      <input type="file" @change="handleFileUpload" accept="image/png, image/jpeg" @blur=validarImagen>
...
export default {
    ...
    computed: {
      ...mapState({
      publicacionesUsuario: state => state.publicacionesU // Asegúrate de que `publicacionesU` está correctamente definido en el store
    }),
    },
  data() {
    return {
    ...
    ...mapActions(['fetchEntradasUsuario', 'agregarEntradaUsuario','eliminarEntradaUsuario','buscarEntradasUsuario','editarEntradaUsuario']), 
    
    irAInicio() {
      this.$router.push('/inicio');
    },
    async buscarEntradasPorUsuario() {
      if (this.busqueda.trim() === '') {
        this.fetchEntradasUsuario();
        return;
      }
      ...
      try {
        const response = await this.buscarEntradasUsuario(this.busqueda);

        if (response && response.length === 0) {
          this.errorBusqueda = true;
          this.mensajeError = 'No se encontró ninguna entrada con ese nombre.';
  ...
  validarFormulario() {
    ...
    return !this.erroresForm.titulo && !this.erroresForm.contenido && !this.erroresForm.categoria && !this.erroresForm.etiquetas && !this.erroresForm.imagenes;
      },
  ...
  validarCategoriaE() {
    ...
    deleteEntrada(titulo) {
       

                this.eliminarEntradaUsuario(titulo).then(() => {
  ...
  handleFileUpload(event) {
    ...
    if (!files) {
      ...
      submitEntrada(entradaForm) {
        if (this.validarFormulario()) {
          ...
          if (this.imagenV) {
            formData.append('imagenes', this.imagenV);
          }
          console.log(this.imagenV);
          this.agregarEntradaUsuario(formData)
            .then(() => {
              console.log('Entrada agregada correctamente');
              this.entradaForm = { titulo: '', contenido: '', categoria: '', etiquetas:'' };
              this.fetchEntradasUsuario(); // Actualiza la lista de entradas después de agregar una nueva
      ...
      submitEntradaE(entradaFormE) {
        if (this.validarFormularioEditar()) {
          ...
          this.editarEntradaUsuario(formDataE)
            .then(() => {
    ...
    created() {
       // this.$store.dispatch('isAdminUser');
        this.fetchEntradasUsuario(); // Carga los usuarios cuando el componente es creado
  }
...
div[v-for="publicacion in publicacionesUsuario"] {
